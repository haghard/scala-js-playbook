...
object Calculator {
  ...
  @dom def renderMainContainer(amount: Var[Double], duration: Var[Int], deposits: Vars[Deposit]) = {
    <div class="row">
      {renderAmountInput(amount).bind}{renderDurationInput(duration).bind}{renderDepositsTable(amount, duration, deposits).bind}
    </div>
  }
  ...
  @dom def renderDeposit(deposit: Deposit, amount: Binding[Double], duration: Binding[Int], onDelete: Deposit => Unit) = {
    <tr>
      <td>
        {deposit.period.toString}
      </td>
      <td>
        {(deposit.interest * 100).toString}
        %</td>
      <td>
  ...
  @dom def renderDepositInput(deposits: Vars[Deposit]) = {
    ...
    <tr>
      <td>
        <input type="number" id="periodInput" data:form="newDeposit" min="1" step="1"/>
      </td>
      <td>
        <input type="number" id="interestInput" data:form="newDeposit" min="0" step="0.01"/>
      </td>
      <td></td>
      <td>
  ...
  def main(selector: String): Unit = {
    com.thoughtworks.binding.dom.render(
      org.scalajs.dom.document.getElementById(selector),
